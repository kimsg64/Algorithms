...
<풀이>
전체 길이의 절반을 가져가므로 최대 n/2종류인데, 폰켓몬이 중복되는 경우에는 set으로 중복을 제거한 뒤 절반을 구해야 한다.
포켓몬의 종류(set)가 n/2 이상이면 상관없는데 이하인 경우에는 가져갈 수 있는 종류가 최대 set의 길이밖에 안 됨

*/
public class Ponkemon {
    public int solution(int[] nums) {
    	int max = nums.length/2;
    	Set<Integer> ponketmonSet = Arrays.stream(nums).boxed().collect(Collectors.toSet());
        int answer = ponketmonSet.size() >= max ? max : ponketmonSet.size();  
        return answer;
    }
}
